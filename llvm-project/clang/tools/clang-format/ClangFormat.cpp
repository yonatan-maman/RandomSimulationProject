...
// Returns true on error.
static bool format(StringRef FileName,StringRef destOutput="") {
  ...
  std::cout << "FORMAT"<< __LINE__ << std::endl;
  Replacements FormatChanges =
      reformat(*FormatStyle, *ChangedCode, Ranges, AssumedFileName, &Status,destOutput);
 std::cout << "FORMAT"<< __LINE__ << std::endl;
...
int main(int argc, const char **argv) {
  ...
  for (const auto &FileName : FileNames) {
    ...
    Error |= clang::format::format(FileName,destOutput);
  }
    std::cout << "main -- error"<< __LINE__ << std::endl;
	return Error ? 1 : 0;
