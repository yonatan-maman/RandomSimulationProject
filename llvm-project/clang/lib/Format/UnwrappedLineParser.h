...
namespace clang {
	namespace format {
		...
		class HelpLexer
		{
			...
			public:
			HelpLexer(const FormatStyle& Style,encoding::Encoding Encoding) : Style(Style),Encoding(Encoding){}
			std::vector<FormatToken *> lexSomething(char* bufferCode, std::vector<FormatToken *>& Toks);
		};
...
public:
  int countLoop;
  UnwrappedLineConsumer() : countLoop(0){}
...
public:
  ...
  UnwrappedLineParser(const FormatStyle &Style,
                      ...
                      unsigned FirstStartColumn,encoding::Encoding Encoding, ArrayRef<FormatToken *> Tokens,
                      UnwrappedLineConsumer &Callback,const SourceManager* sm,std::ostream& output);
  	std::list<std::list<std::string>> listInputs;
  void parse();
private:
	const SourceManager* SM;
	void printLines();
	void createUnwrappedLineNodeList (std::string&  bufferCode, HelpLexer helpLexer);
  void reset();
  void parseFile();
  void parseLevel(bool HasOpeningBrace);
  void parseBlock(bool MustBeDeclaration, bool AddLevel = true,
                  bool MunchSemi = true ,bool isLoop = false);
  ...
  void MYreadToken(FormatToken *tok,int LevelDifference = 0);
  void MYnextToken(FormatToken *tok,int LevelDifference = 0);
  void parseParens(bool isInput = false);
  ...
  UnwrappedLineConsumer &Callback;
	inline void printToken(FormatToken *Tok);
