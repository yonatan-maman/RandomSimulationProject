...
class UnwrappedLine;
std::vector<FormatToken *> HelpLexer::lexSomething(char* bufferCode, std::vector<FormatToken *>& Toks)
{
	static int countTempFiles = 0 ;
	std::string fileName = "/tmp/temp" + std::to_string(countTempFiles) + ".cpp" ;
	std::cout << fileName << "-----------------------" <<std::endl;
	std::ofstream code (fileName);
	...
	//std::cout << bufferCode << std::endl;
			std::cout << "3333"<<std::endl;

	std::vector<tooling::Range> Ranges;
			std::cout << "4444"<<std::endl;

	Ranges.push_back(tooling::Range(0, strlen(bufferCode)));
				std::cout << "5555"<<std::endl;
	Environment Env (bufferCode, fileName, Ranges, 0,0, 0);
	...
	std::cout << "%%%%%%%%%%%%%%%%%%%%%%%%%%" <<std::endl;
	for (std::vector<FormatToken *>::iterator it = Toks.begin() ; it != Toks.end(); ++it){
		std::cout << (*it)->Tok.getName() << "----"<<std::endl;
	}
	std::cout << "%%%%%%%%%%%%%%%%%%%%%%%%%%%"<<std::endl;
	return Toks;	
	//remove(fileName);
}
void UnwrappedLineParser::printLines()
{
	std::cout <<" ***********before***********"<< std::endl;
	for	(SmallVectorImpl<UnwrappedLine>::iterator L = Lines.begin(); L != Lines.end(); ++L ){
		for (std::list<UnwrappedLineNode>::iterator it = L->Tokens.begin() ; it != L->Tokens.end(); ++it){
			std::cout << ((it->Tok)->Tok).getName() << "    " ;
		}
		std::cout << std::endl;
	}
		std::cout <<" ***********before***********"<< std::endl;
...
void UnwrappedLineParser::createUnwrappedLineNodeList (std::string& bufferCode, HelpLexer helpLexer)
{
	std::cout << bufferCode;
	std::vector<FormatToken *> vector;
	helpLexer.lexSomething(strdup(bufferCode.c_str()),vector);
	std::cout << "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$" << std::endl;
	for (std::vector<FormatToken *>::iterator it = vector.begin() ; it != vector.end(); ++it){
		std::cout << "----";
		std::cout << (*it)->Tok.getName() << std::endl;
	}
	std::cout << "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$"<< std::endl;	
...
UnwrappedLineParser::UnwrappedLineParser(const FormatStyle &Style,
                                         ...
                                         ArrayRef<FormatToken *> Tokens,
                                         UnwrappedLineConsumer &Callback,const SourceManager* sm,std::ostream& output)
    : Line(new UnwrappedLine), MustBreakBeforeNextToken(false),
      CurrentLines(&Lines), Style(Style), Keywords(Keywords),
      CommentPragmasRegex(Style.CommentPragmas),SM(sm),helpLexer(Style,Encoding),Tokens(nullptr), output(output),isThereMain(false),
...
void UnwrappedLineParser::parseCSharpGenericTypeConstraint() {
			  std::cout << "parseCSharpGenericTypeConstraint()" << std::endl;
...
void UnwrappedLineParser::parseCSharpAttribute() {
				  std::cout << "parseCSharpAttribute()" << std::endl;
...
void UnwrappedLineParser::calculateBraceTypes(bool ExpectClassBody) {
						  std::cout << "calculateBraceTypes()" << std::endl;
...
size_t UnwrappedLineParser::computePPHash() const {
	std::cout << "computePPHash()" << std::endl;
  size_t h = 0;
...
void UnwrappedLineParser::parseBlock(bool MustBeDeclaration, bool AddLevel,
                                     bool MunchSemi,bool isLoop) {
	    //std::cout << "parseBlock start" << std::endl;
  ...
  if (PPStartHash == PPEndHash) {
    ...
    if (OpeningLineIndex != UnwrappedLine::kInvalidIndex) {
      ...
      (*CurrentLines)[OpeningLineIndex].MatchingClosingBlockLineIndex =
  	    ...
  	    std::cout << "parseBlock finish" << std::endl;
...
void UnwrappedLineParser::parseChildBlock() {
		std::cout << "parseChildBlock()" << std::endl;
...
void UnwrappedLineParser::parsePPDirective() {
			std::cout << "parsePPDirective()" << std::endl;
...
void UnwrappedLineParser::conditionalCompilationCondition(bool Unreachable) {
				std::cout << "pconditionalCompilationCondition(bool Unreachable)" << std::endl;
...
void UnwrappedLineParser::conditionalCompilationStart(bool Unreachable) {
	std::cout << "conditionalCompilationStart" << std::endl;
  ++PPBranchLevel;
...
void UnwrappedLineParser::conditionalCompilationAlternative() {
		std::cout << "conditionalCompilationAlternative()" << std::endl;
...
void UnwrappedLineParser::conditionalCompilationEnd() {
			std::cout << "conditionalCompilationEnd()" << std::endl;
...
void UnwrappedLineParser::parsePPElIf() {		std::cout << "parsePPElIf()" << std::endl;
 parsePPElse(); }

void UnwrappedLineParser::parsePPEndIf() {
	std::cout << "parsePPEndIf()" << std::endl;
...
void UnwrappedLineParser::parsePPDefine() {
		std::cout << "parsePPDefine()" << std::endl;
...
void UnwrappedLineParser::parsePPUnknown() {
			std::cout << "parsePPUnknown()" << std::endl;
...
void UnwrappedLineParser::readTokenWithJavaScriptASI() {
				std::cout << "readTokenWithJavaScriptASI()" << std::endl;
...
void UnwrappedLineParser::parseStructuralElement() {
	std::cout<< "parseStructuralElement()" << std::endl; 
  ...
  case tok::kw_inline:
    ...
    if (FormatTok->Tok.is(tok::kw_namespace)) {
					std::cout << "kw_namespace" << std::endl;
    ...
    break;
  case tok::identifier:
	std::cout << "case tok::identifier -0-" << std::endl;
    if (FormatTok->is(TT_ForEachMacro)) {
			std::cout << "TT_ForEachMacro" << std::endl;

      parseForOrWhileLoop();
      return;
    }
    if (FormatTok->is(TT_MacroBlockBegin)) {
		std::cout << "TT_MacroBlockBegin" << std::endl;
      parseBlock(/*MustBeDeclaration=*/false, /*AddLevel=*/true,
                 /*MunchSemi=*/false);
      return;
    }
    if (FormatTok->is(Keywords.kw_import)) {
		std::cout << "Keywords.kw_import" << std::endl;
      if (Style.Language == FormatStyle::LK_JavaScript) {
        parseJavaScriptEs6ImportExport();
        return;
      }
      if (Style.Language == FormatStyle::LK_Proto) {
		  					std::cout << "LK_Proto" << std::endl;
    ...
    if (Style.isCpp() && FormatTok->is(TT_StatementMacro)) {
				std::cout << "TT_StatementMacro" << std::endl;

      parseStatementMacro();
      return;
    }
    if (Style.isCpp() && FormatTok->is(TT_NamespaceMacro)) {
		std::cout << "TT_NamespaceMacro" << std::endl;
      parseNamespace();
      return;
    }
  ...
  do {
    ...
    switch (FormatTok->Tok.getKind()) {
    case tok::at:
		std::cout << "tok::at" << std::endl;
      nextToken();
      if (FormatTok->Tok.is(tok::l_brace)) {
		std::cout << "tok::l_brace" << std::endl;
        nextToken();
        parseBracedList();
        break;
      ...
      switch (FormatTok->Tok.getObjCKeywordID()) {
		std::cout << "FormatTok->Tok.getObjCKeywordID()" << std::endl;
      case tok::objc_public:
      case tok::objc_protected:
      case tok::objc_package:
      case tok::objc_private:
		std::cout << "tok::objc_private" << std::endl;
        return parseAccessSpecifier();
      case tok::objc_interface:
      case tok::objc_implementation:
      	std::cout << "tok::objc_implementation" << std::endl;
        return parseObjCInterfaceOrImplementation();
      case tok::objc_protocol:
        std::cout << "tok::objc_protocol" << std::endl;
        if (parseObjCProtocol())
          return;
        break;
      case tok::objc_end:
      	std::cout << "tok::objc_end" << std::endl;
        return; // Handled by the caller.
      case tok::objc_optional:
      case tok::objc_required:
		std::cout << "tok::objc_required" << std::endl;
        nextToken();
        addUnwrappedLine();
        return;
      case tok::objc_autoreleasepool:
      	std::cout << "tok::objc_autoreleasepool" << std::endl;
        nextToken();
        if (FormatTok->Tok.is(tok::l_brace)) {
			std::cout << "tok::l_brace" << std::endl;
      ...
      case tok::objc_synchronized:
        std::cout << "tok::objc_synchronized" << std::endl;
        nextToken();
        if (FormatTok->Tok.is(tok::l_paren)){
          // Skip synchronization object
          std::cout << "tok::l_paren" << std::endl;
          parseParens();
	  }
        if (FormatTok->Tok.is(tok::l_brace)) {
			std::cout << "tok::l_brace" << std::endl;
        ...
        return;
      case tok::objc_try:
      	std::cout << "tok::objc_try" << std::endl;
    ...
    case tok::kw_enum:
      ...
      break;
    case tok::kw_typedef:
    		std::cout << "tok::kw_typedef" << std::endl;
      nextToken();
    ...
    case tok::kw_union:
    case tok::kw_class:
        		std::cout << "case tok::kw_struct:  case tok::kw_union:  case tok::kw_class:" << std::endl;
    ...
    case tok::period:
      ...
      break;
    case tok::semi:
		std::cout << "tok::semi:" << std::endl;
      nextToken();
      addUnwrappedLine();
      return;
    case tok::r_brace:
			std::cout << "tok::r_brace" << std::endl;
      addUnwrappedLine();
      return;
    case tok::l_paren:
    	std::cout << "tok::l_paren" << std::endl;
    ...
    case tok::kw_operator:
      std::cout << "tok::kw_operator:" << std::endl;
      nextToken();
      if (FormatTok->isBinaryOperator())
        nextToken();
      break;
    case tok::caret:
          std::cout << "tok::caret:" << std::endl;
      nextToken();
    ...
    case tok::kw_try:
      ...
      return;
    case tok::identifier: {
		std::cout << "tok::identifier --1--:" << std::endl;
    ...
    case tok::equal:
      ...
      break;
    case tok::l_square:
		std::cout << "tok::l_square" << std::endl;
      parseSquare();
      break;
    case tok::kw_new:
		std::cout << "tok::kw_new:" << std::endl;
      parseNew();
      break;
    default:
    	std::cout << "default:" << std::endl;
...
bool UnwrappedLineParser::tryToParsePropertyAccessor() {
		std::cout<< "tryToParsePropertyAccessor()" << std::endl; 
...
bool UnwrappedLineParser::tryToParseLambda() {
	std::cout << "tryToParseLambda()" <<std::endl;
  if (!Style.isCpp()) {
    nextToken();
    return false;
  }
...
bool UnwrappedLineParser::tryToParseLambdaIntroducer() {
		std::cout << "tryToParseLambdaIntroducer()" <<std::endl;
...
void UnwrappedLineParser::tryToParseJSFunction() {
			std::cout << "tryToParseJSFunction()" <<std::endl;
...
bool UnwrappedLineParser::tryToParseBracedList() {
	std::cout << "tryToParseBracedList()" << std::endl; 
...
bool UnwrappedLineParser::parseBracedList(bool ContinueOnSemicolons,
  ...
  bool HasError = false;
	std::cout << "parseBracedList()" << std::endl; 
...
void UnwrappedLineParser::parseParens(bool isInput) {
		std::cout << "parseParens()" << std::endl;
		std::cout << isInput ;
	if(isInput){
			listInputs.push_back(std::list<std::string>());	
			listInputs.back().push_back("");		
		}
  ...
  do {
	...
	case tok::identifier:
		...
		if(isInput)
			listInputs.back().back() +=" "+ FormatTok->TokenText.str();
    ...
    default:
		...
		if(isInput && FormatTok->is(tok::comma))
			listInputs.back().push_back(" ");
		else if (isInput)
			listInputs.back().back() +=FormatTok->TokenText.str()+" ";
...
void UnwrappedLineParser::parseIfThenElse() {
	std::cout<<"parseIfThenElse" << std::endl;
...
void UnwrappedLineParser::parseTryCatch() {
		std::cout<<"parseTryCatch" << std::endl;
...
void UnwrappedLineParser::parseNamespace() {
	std::cout << "parseNamespace()" << std::endl;
...
void UnwrappedLineParser::parseForOrWhileLoop() {
  ...
  if (FormatTok->Tok.is(tok::l_paren))
    parseParens();
	//std::string s2 (std::string("g_statistics::plusplus(__FILE__ ,")+ std::to_string(row)+std::string(",")+ std::to_string(column) +std::string(");"));

  if (FormatTok->Tok.is(tok::l_brace)) {  
	CompoundStatementIndenter Indenter(this, Style, Line->Level);
	//createUnwrappedLineNodeList(strdup("int a = 0;"),helpLexer);
	addUnwrappedLine(); 
	nextToken();
    ...
    output <<std::endl<< "}";
    //s2 += "}";
    //createUnwrappedLineNodeList((s2),helpLexer); 
...
void UnwrappedLineParser::parseCaseLabel() {
				std::cout <<"parseCaseLabel" <<std::endl;
...
void UnwrappedLineParser::parseAccessSpecifier() {
	std::cout << "parseAccessSpecifier" << std::endl;
  nextToken();
...
bool UnwrappedLineParser::tryToParseSimpleAttribute() {
	std::cout << "tryToParseSimpleAttribute" <<std::endl;
...
void UnwrappedLineParser::parseJavaEnumBody() {
		std::cout << "parseJavaEnumBody" <<std::endl;
...
void UnwrappedLineParser::parseObjCMethod() {
				std::cout << "parseObjCMethod" <<std::endl;
...
void UnwrappedLineParser::parseObjCProtocolList() {
					std::cout << "parseObjCProtocolList" <<std::endl;
...
void UnwrappedLineParser::parseObjCUntilAtEnd() {
  do {
	  					std::cout << "parseObjCUntilAtEnd" <<std::endl;
...
void UnwrappedLineParser::parseObjCInterfaceOrImplementation() {
	std::cout << "parseObjCInterfaceOrImplementation" << std::endl;
...
void UnwrappedLineParser::parseObjCLightweightGenerics() {
		std::cout << "parseObjCLightweightGenerics" << std::endl;
...
bool UnwrappedLineParser::parseObjCProtocol() {
			std::cout << "parseObjCProtocol" << std::endl;
...
void UnwrappedLineParser::parseJavaScriptEs6ImportExport() {
	std::cout << "parseJavaScriptEs6ImportExport" << std::endl;
...
void UnwrappedLineParser::parseStatementMacro() {
		std::cout << "parseStatementMacro" << std::endl;
...
void UnwrappedLineParser::addUnwrappedLine() {
	...
	std::cout << "addUnwrappedLine()  start" << std::endl;
  if (Line->Tokens.empty()){
	  std::cout << "addUnwrappedLine()  finish" << std::endl;
    return;
}
  LLVM_DEBUG({
...
bool UnwrappedLineParser::eof() const { 		std::cout << "eof" << std::endl;
return FormatTok->Tok.is(tok::eof); }

bool UnwrappedLineParser::isOnNewLine(const FormatToken &FormatTok) {
			std::cout << "isOnNewLine" << std::endl;
...
continuesLineCommentSection(const FormatToken &FormatTok,
  			...
  			std::cout << "continuesLineCommentSection" << std::endl;
...
void UnwrappedLineParser::flushComments(bool NewlineBeforeNext) {
	  			std::cout << "flushComments" << std::endl;
...
void UnwrappedLineParser::nextToken(int LevelDifference) {
  ...
  if (Style.Language != FormatStyle::LK_JavaScript)
    ...
    readTokenWithJavaScriptASI();
      	std::cout << "----FormatTok->Previous----" <<std::endl;
  FormatTok->Previous = Previous;
  	std::cout << "finishNextToken" <<std::endl;
}

void UnwrappedLineParser::distributeComments(
    const SmallVectorImpl<FormatToken *> &Comments,
    const FormatToken *NextTok) {
			std::cout << "distributeComments" <<std::endl;
...
inline void UnwrappedLineParser::printToken(FormatToken *Tok)
{
	output << whiteSpaceAdding(*SM,Tok->WhitespaceRange)<< Tok->TokenText.str();
}
void UnwrappedLineParser::pushToken(FormatToken *Tok) {
					std::cout << "pushToken" <<std::endl;
  std::cout << "-----" <<std::endl;
...
void UnwrappedLineParser::MYnextToken(FormatToken *tok,int LevelDifference) {
  	//std::cout << tok->Tok.getName();
  if(FormatTok == nullptr){
		  FormatTok = tok;
		  return;
	}
  if (eof())
  ...
  FormatToken *Previous = FormatTok;
  if (Style.Language != FormatStyle::LK_JavaScript)
	MYreadToken(tok,LevelDifference);
  else
	readTokenWithJavaScriptASI();
	FormatTok->Previous = Previous;
}
void UnwrappedLineParser::MYreadToken(FormatToken *tok,int LevelDifference) {
  SmallVector<FormatToken *, 1> Comments;
  do {
    FormatTok = tok;
    assert(FormatTok);
