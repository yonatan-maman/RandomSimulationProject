...
public:
  Formatter(const Environment &Env, const FormatStyle &Style,
            FormattingAttemptStatus *Status,StringRef destOutput="")
      : TokenAnalyzer(Env, Style,destOutput), Status(Status) {  std::cout <<"-formater--"<< destOutput.str() << std::endl;}
  ...
  analyze(TokenAnnotator &Annotator,
    ...
    return std::make_pair(Result, 0 /*Penalty*/);
  }
...
std::pair<tooling::Replacements, unsigned> internal::reformat(const FormatStyle &Style, StringRef Code,
...
reformat(const FormatStyle &Style, StringRef Code,
         ...
         unsigned NextStartColumn, unsigned LastStartColumn, StringRef FileName,
         FormattingAttemptStatus *Status, StringRef destOutput) {
    std::cout << destOutput.str() << std::endl;
  ...
  if (Expanded.Language == FormatStyle::LK_JavaScript && isMpegTS(Code))
    return {tooling::Replacements(), 0};
  std::cout <<"-internal--"<< destOutput.str() << std::endl;
  ...
  Passes.emplace_back([&](const Environment &Env) {
    return Formatter(Env, Expanded, Status,destOutput).process();
  });
...
tooling::Replacements reformat(const FormatStyle &Style, StringRef Code,
  ...
  return internal::reformat(Style, Code, Ranges,
                            /*FirstStartColumn=*/0,
                            /*NextStartColumn=*/0,
                            /*LastStartColumn=*/0, FileName, Status ,destOutput)
      .first;
}
...
tooling::Replacements reformat(const FormatStyle &Style, StringRef Code,
                               ArrayRef<tooling::Range> Ranges,
                               StringRef FileName, bool *IncompleteFormat,StringRef destOutput) {
  FormattingAttemptStatus Status;
  auto Result = reformat(Style, Code, Ranges, FileName, &Status,destOutput);
