...
TokenAnalyzer::TokenAnalyzer(const Environment &Env, const FormatStyle &Style,StringRef destOutput)
    : Style(Style), Env(Env),destOutput(destOutput),
      AffectedRangeMgr(Env.getSourceManager(), Env.getCharRanges()),
      UnwrappedLines(1),
      Encoding(encoding::detectEncoding(
          Env.getSourceManager().getBufferData(Env.getFileID()))) {
  std::cout <<"---"<<this->destOutput.str() << std::endl;
...
static void makeDirectories(std::string sourcePath ,std::string AddingPath)
{
	size_t index = AddingPath.find('/',0);
	while(index != std::string::npos)
	{
		std::string partDirectoryPath = sourcePath+ AddingPath.substr(0,index);
		std::cout << sourcePath << "----" << AddingPath.substr(0,index);
		std::cout <<std::endl<<partDirectoryPath<<std::endl;
		mkdir(partDirectoryPath.c_str(),S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
		index = AddingPath.find('/',index+1);
	}
}


static std::string createInclude(std::string Path)
{
	std::string s;
	std::cout << Path << std::endl;
	size_t index = Path.find('./',0)+2;
...
std::pair<tooling::Replacements, unsigned> TokenAnalyzer::process() {
  ...
  SmallVector<FormatToken *, 10> Tokens(Toks.begin(), Toks.end());
	std::cout << this->destOutput.str() <<std::endl;
  std::string directoryPath = this->destOutput.str();
    std::cout <<"******"<<directoryPath<<"*******"<<std::endl;
  makeDirectories(directoryPath,"/files/"+Env.FileName.str());
  std::string fullDirectoryPathWithoutName = directoryPath+ Env.FileName.str().substr(0,Env.FileName.str().find_last_of ('/'));
  //std::cout << (directoryPath+Env.FileName.str(Env.FileName.str())+"/../");
  std::ofstream newCode (directoryPath +"/files/"+Env.FileName.str() );
  newCode << "#include \""<<createInclude(Env.FileName.str())<<"\"" << std::endl;
  UnwrappedLineParser Parser(Style, Lex.getKeywords(),
                             Env.getFirstStartColumn(),Encoding, Tokens, *this , &(Env.getSourceManager()), newCode);
  Parser.parse();
  assert(UnwrappedLines.rbegin()->empty());
  mkdir((directoryPath +"/files/temp").c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
  std::ofstream inputs (directoryPath +"/files/temp/tempInputs");
  ...
  if(Parser.isThereMain)
	  inputs << "\n" << Env.FileName.str()<< "-";
    std::cout << std::endl << "------------"<<std::endl << "------------"<<std::endl << "------------" <<"------------"<< Env.FileName.str()  <<"------"<<std::endl << std::endl;
  std::cout << (directoryPath+"temp") << std::endl;
  std::cout << fullDirectoryPathWithoutName <<std::endl;
