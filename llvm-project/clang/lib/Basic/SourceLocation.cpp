...
void clang::getRawcolumnLoop(unsigned int& column,unsigned int& row,  SourceLocation Loc , const SourceManager &SM)
{
...
std::string llvm::whiteSpaceAdding(const clang::SourceManager &SM,clang::SourceRange range)
{
	PresumedLoc PLoc1 = SM.getPresumedLoc(range.getBegin());
	PresumedLoc PLoc2 = SM.getPresumedLoc(range.getEnd());
	std::string s;
	for(int i = PLoc1.getLine() ; i < PLoc2.getLine();i++,s+="\n");
	for(int i = PLoc1.getColumn() ; i < PLoc2.getColumn();i++,s+=" ");
	return s;
}
